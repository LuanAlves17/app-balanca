...
function EmbarqueProvider({ children }) {
    ...
    function acceptOn(data) {
        setData((prevData) => {
            ...
            novaArray[indiceData] = { ...novaArray[indiceData], accepted: true, acceptedAt: `${dateref.getFullYear()}/${dateref.getMonth()}/${dateref.getDay()} ${dateref.getHours()}:${dateref.getMinutes()}` };

            return novaArray;
        })
    }

    const value = useMemo(() => ({ data, acceptOn }), [data]);
